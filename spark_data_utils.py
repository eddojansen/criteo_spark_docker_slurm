...
def getTotalSize(spark, str_path):
    sc = spark.sparkContext
    config = sc._jsc.hadoopConfiguration()
    path = sc._jvm.org.apache.hadoop.fs.Path(str_path)
    uri = sc._jvm.java.net.URI(str_path)
    fs = sc._jvm.org.apache.hadoop.fs.FileSystem.get(uri, config)
    filestat = fs.listStatus(path)
    size = 0
    for stat in filestat:
        if stat.isDirectory():
            size += getTotalSize(spark, stat.getPath())
        else:
            size += stat.getLen()
    return size
...
def would_broadcast(spark, str_path):
    sc = spark.sparkContext
    sum = getTotalSize(spark, str_path)
    sql_conf = spark.conf
    autoBroadcastJoinThreshold = sc._jvm.org.apache.spark.util.Utils.byteStringAsBytes(sql_conf.get("spark.sql.autoBroadcastJoinThreshold"))
    fileCompressionFactor = float(sql_conf.get("spark.sql.sources.fileCompressionFactor"))
    cutoff = autoBroadcastJoinThreshold * fileCompressionFactor
    return sum <= cutoff
